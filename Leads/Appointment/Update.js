...
export default class AppointmentUpdate extends Component {
	...
	getInitialData = () => {
		if (this.props.location.state.booking) this.setState({ id: this.props.location.state.booking.id });
		else this.setState({ id: this.props.location.state.id });

		if (this.props.location.state.status) this.setState({ status: this.props.location.state.status });
		else this.setState({ status: this.props.location.state.booking.status });
	};
	...
	updateStatus = () => {
		...
		api.request('patch', `https://roofr.gotomy.dev/api/v1/bookings/${id}`, status).then((response) => {
